...
def check_id(folder_id):
    ...
    try:
        ...
        title = title_match.group(1).replace(" - Google Drive", "") if title_match else "unknown"
        return folder_id, title
    except:
        return folder_id, "error"

print("Checking subfolder titles on Google Drive...")
with concurrent.futures.ThreadPoolExecutor(max_workers=10) as executor:
    results = list(executor.map(check_id, ids))

print("\nTitles found:")
for folder_id, title in results:
    if "error" not in title and "unknown" not in title:
