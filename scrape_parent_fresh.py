...
try:
    print("Fetching fresh parent folder HTML...")
    with urllib.request.urlopen(req) as response:
        html = response.read().decode('utf-8')
    print("Successfully fetched fresh parent folder HTML.")
    
    with open("parent_phase_scans_fresh.html", "w", encoding="utf-8") as f:
        f.write(html)
        
    # Search for "v0.34" or "NMR" or "records"
    print("\nSearching for files/subfolders in parent_phase_scans_fresh.html...")
    for word in ["v0.34", "NMR", "records", "NMR_records", "NMRexp"]:
